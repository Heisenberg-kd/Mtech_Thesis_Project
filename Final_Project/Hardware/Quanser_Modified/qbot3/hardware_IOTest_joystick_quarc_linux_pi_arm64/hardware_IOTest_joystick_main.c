...
typedef RT_MODEL_hardware_IOTest_joys_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * hardware_IOTest_joystick(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(hardware_IOTest_joystick_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: hardware_IOTest_joystick -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
      ...
      if (result == 0) {
        ...
        if (result == 0) {
          ...
          if (result == 0) {
            ...
            {
              pstream_shutdown(hardware_IOTest_joystick_DW.StreamClient_Stream);
            }
