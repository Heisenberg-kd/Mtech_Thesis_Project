...
typedef RT_MODEL_camera_lidar_IOTest_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * camera_lidar_IOTest(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(camera_lidar_IOTest_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: camera_lidar_IOTest -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
