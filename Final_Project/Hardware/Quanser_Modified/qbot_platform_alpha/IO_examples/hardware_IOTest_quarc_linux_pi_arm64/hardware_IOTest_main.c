...
typedef RT_MODEL_hardware_IOTest_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * hardware_IOTest(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(hardware_IOTest_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: hardware_IOTest -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
      ...
      if (result == 0) {
        ...
        if (result == 0) {
          ...
          if (result == 0) {
            ...
            {
              pstream_shutdown(hardware_IOTest_DW.StreamClient_Stream);
            }
